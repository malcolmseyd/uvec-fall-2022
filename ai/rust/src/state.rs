...
impl State {
    ...
    fn game_step_vertical(&self, row: usize, column: usize) -> State {
        ...
        && new_state.h_lines[row + 1][column - 1] != 0 // bottom line
        {
            // Bottom line
        ...
        && new_state.h_lines[row + 1][column] != 0 // bottom line
        {
            // bottom line
            new_state.score.value += match self.p1_turn {
                true => 1,
    ...
    fn game_step_horizontal(&self, row: usize, column: usize) -> State {
        ...
        && self.v_lines[row - 1][column + 1] != 0 // right line
        {
            // right line
            new_state.score.value += match self.p1_turn {
                true => 1,
        ...
        && self.v_lines[row][column + 1] != 0 // right line
        {
            // right line
            new_state.score.value += match self.p1_turn {
                true => 1,
